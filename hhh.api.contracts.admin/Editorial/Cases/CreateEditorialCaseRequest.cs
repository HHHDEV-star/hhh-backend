...
/// <summary>
/// 編輯部 - 新增個案請求
/// (對應舊版 hhh-api/.../base/v1/Cases.php → index_post → case_model::insert_case_data())
/// 舊版必填欄位:onoff, caption, type, style, condition (見 PHP $chk_empty)
/// </summary>
public class CreateEditorialCaseRequest
{
    ...
    /// <summary>上架日期(空白時 service 會預設為今天,對應舊 PHP 行為)</summary>
    public DateOnly? Sdate { get; set; }

    /// <summary>設計師 ID(舊 PHP 沒列入必填)</summary>
    public uint HdesignerId { get; set; }

    /// <summary>
    /// Tag(編輯部慣例:由前端組成 style + ',' + type + ',' + condition,服務端原樣存)。
    /// 為避免不一致,若空白會由 service 自動以同樣規則組起來。
    /// </summary>
    [StringLength(150)]
